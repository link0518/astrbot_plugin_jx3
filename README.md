...
- 支持设置插件使用范围：全部开放、白名单、黑名单三种模式，名单可精确到群，私聊可独立开关。
...
| 本地 SQLite | `core/sqlite.py`、`core/bilei_data.py`、`core/cache.py` | 会话配置、心法别名、避雷记录、事件订阅、使用范围名单、接口缓存及图片缓存索引 |
...
会话控制、区服绑定、区服/心法别名、避雷记录、事件订阅及缓存配置均保存在本地 SQLite。修改 WebSocket 地址、事件版令牌或 TLS 证书验证开关后需要重新加载插件。
...
避雷数据按 AstrBot 的 `unified_msg_origin` 严格隔离，一个会话无法查看或操作其他会话的记录。升级前已有的避雷记录会自动保留在普通会话不可访问的“历史公共数据”区。避雷功能没有会话内的用户角色校验；在“会话控制”放行的同一会话中，只要能够触发插件指令，当前实现就允许新增、修改和删除该会话的记录。如需限制到指定管理员，应在 AstrBot 或消息平台层配置权限，也可以先在插件管理页“使用范围”中将模式切换为白名单，只对指定群开放。
...
| 使用范围 | 在全部开放、群白名单和群黑名单之间切换，名单支持完整会话 ID 或纯群号；私聊可独立开关，被拒会话可配置回复提示；列出最近活跃的群并可一键加入名单 |

“会话控制”（上游）与“使用范围”（本地）两套访问策略并存且默认均为全部放行，一条消息需同时通过两者才会被处理；使用范围除指令外同样约束实时事件推送，被停用的群不再收到任何订阅事件。

“使用范围”页签的名单条目支持完整会话 ID（AstrBot `unified_msg_origin`）或纯群号；群内触发过插件指令的会话会记录在本地 SQLite，页面会列出最近活跃的群并支持一键加入名单，白名单模式下无需手动抄群号。私聊是否可用由独立开关控制，不随群名单模式变化。被拒会话触发指令时默认静默忽略，也可开启回复提示。
...
- `access_config`：使用范围配置（模式、私聊开关、被拒提示开关）。
- `access_entries`：使用范围名单条目（会话 ID/群号与备注）。
...
4. 命中指令后先记录会话来源（管理页“最近活跃的群”数据来源），再按使用范围判断本会话/本群是否可用：全部开放直接放行；白名单/黑名单按名单匹配群；私聊按独立开关判定；被拒时按配置静默忽略或回复提示。随后再经会话控制二次过滤，两套策略默认均为全部放行。
...
| 文件 | 生命周期 | 内容 |
| --- | --- | --- |
| AstrBot 插件数据目录下的 `local_data.db` | 运行时创建和维护 | 心法与别名、避雷记录、会话控制、使用范围名单、会话区服绑定、区服别名、事件订阅及基础数据缓存 |
...
每条消息按 `action` 识别事件，正文优先读取 `detail`，同时兼容 `data`。分发时先应用会话控制策略并按使用范围过滤被停用的会话，再联查会话区服绑定：正文包含 `server` 时仅保留同区服或未绑定区服的接收会话，正文不含 `server` 时不做区服过滤。之后只有会话的 `enabled=1` 且对应 `action_编号=1` 时才发送。会话 ID 直接取当前消息的 `unified_msg_origin`。
...
│   ├── access_control.py    # 插件使用范围（群启用控制）与活跃会话记录
