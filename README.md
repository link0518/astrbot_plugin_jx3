...
“授权管理”除指令外同样约束实时事件推送，被停用的群不再收到任何订阅事件。群内触发指令时会通过平台 get_group_info 接口自动抓取并缓存群名（每群 24 小时最多重取一次，仅支持 aiocqhttp 平台，失败静默跳过），名单与“最近活跃的群”优先展示群名、以小字附注 ID，拿不到群名时回退显示 ID。
