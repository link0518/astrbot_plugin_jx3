...
- 支持设置插件授权管理：全部开放、白名单、黑名单三种模式，名单可精确到群，私聊可独立开关。
...
- 内置 98 个中文触发指令和 51 个页面片段，通过公共布局与样式在本地组装为完整 HTML，并附带通用、沙盘、门派/心法和奇遇图标资源。
...
| 本地 SQLite | `core/sqlite.py`、`core/bilei_data.py`、`core/cache.py` | 会话配置、心法别名、避雷记录、事件订阅、授权管理名单、接口缓存及图片缓存索引 |
...
| `matplotlib` | 当前依赖清单保留的绘图依赖；v3.4.8 业务代码未直接导入 |
...
授权管理、区服绑定、区服/心法别名、避雷记录、事件订阅及缓存配置均保存在本地 SQLite。修改 WebSocket 地址、事件版令牌或 TLS 证书验证开关后需要重新加载插件。
...
避雷数据按 AstrBot 的 `unified_msg_origin` 严格隔离，一个会话无法查看或操作其他会话的记录。升级前已有的避雷记录会自动保留在普通会话不可访问的“历史公共数据”区。避雷功能没有会话内的用户角色校验；只要能够触发插件指令，当前实现就允许新增、修改和删除该会话的记录。如需限制到指定管理员，应在 AstrBot 或消息平台层配置权限，也可以先在插件管理页“授权管理”中将模式切换为白名单，只对指定群开放。
...
| 页签 | 当前功能 |
| --- | --- |
| 授权管理 | 默认页签；在全部开放、群白名单和群黑名单之间切换，名单支持完整会话 ID 或纯群号，会话列以「群名 + 小字 ID」双行展示；私聊可独立开关，被拒会话可配置回复提示；列出最近活跃的群（展示群名）并可一键加入名单 |
...
“授权管理”除指令外同样约束实时事件推送，被停用的群不再收到任何订阅事件。群内触发指令时会通过平台 get_group_info 接口自动抓取并缓存群名（每群 24 小时最多重取一次，仅支持 aiocqhttp 平台，失败静默跳过），名单与“最近活跃的群”优先展示群名、以小字附注 ID，拿不到群名时回退显示 ID；「最近活跃的群」右上角的「抓取群名」按钮可对已知群（含名单中的纯数字群号，即使从未触发过指令）批量补抓，需插件运行期间收到过任一 QQ 消息以建立连接；群名独立存储于 group_names 表，不依赖活跃记录。

“授权管理”页签的名单条目支持完整会话 ID（AstrBot `unified_msg_origin`）或纯群号；群内触发过插件指令的会话会记录在本地 SQLite，页面会列出最近活跃的群并支持一键加入名单，白名单模式下无需手动抄群号。私聊是否可用由独立开关控制，不随群名单模式变化。被拒会话触发指令时默认静默忽略，也可开启回复提示。

事件推送配置保存后立即用于后续事件分发，仍受授权管理模式和绑定区服限制。新增时可以选择已有会话或输入完整的 AstrBot 会话 ID，重复新增会提示编辑已有配置；编辑时会话 ID 只读。空事件选择表示不接收任何事件；删除仅移除该会话的推送配置，保留区服绑定、访问名单和其他会话数据。免费事件与令牌事件分组展示，令牌事件仍需配置 `jx3api_wss_token`。
...
4. 创建本地数据库、两个数据服务、心法别名服务、避雷服务、事件推送服务、WebUI 服务和消息构建器。
...
- `access_config`：授权管理配置（模式、私聊开关、被拒提示开关）。
- `access_entries`：授权管理名单条目（会话 ID/群号与备注）。
- `access_sessions`：最近触发过插件指令的会话记录（群/私聊类型与群号）。
...
4. 命中指令后先记录会话来源（管理页“最近活跃的群”数据来源），再按授权管理判断本会话/本群是否可用：全部开放直接放行；白名单/黑名单按名单匹配群；私聊按独立开关判定；被拒时按配置静默忽略或回复提示。
5. 允许后停止事件继续传播；同会话、同发送者、同指令、同参数的在途请求会被去重并提示，避免渲染发图期间连发堆积。
6. `_prepare_server_args()` 和 `_prepare_kungfu_args()` 根据处理器参数名统一补齐区服绑定并解析区服、心法别名；`_call_with_auto_args()` 通过 `inspect.signature()` 读取参数签名，按注解转换 `int` 和 `float` 后调用处理器。
7. 找不到指令时忽略消息。
...
| 文件 | 生命周期 | 内容 |
| --- | --- | --- |
| AstrBot 插件数据目录下的 `local_data.db` | 运行时创建和维护 | 心法与别名、避雷记录、授权管理名单、会话区服绑定、区服别名、事件订阅及基础数据缓存 |
...
每条消息按 `action` 识别事件，正文优先读取 `detail`，同时兼容 `data`。分发时按授权管理过滤被停用的会话，再联查会话区服绑定：正文包含 `server` 时仅保留同区服或未绑定区服的接收会话，正文不含 `server` 时不做区服过滤。之后只有会话的 `enabled=1` 且对应 `action_编号=1` 时才发送。会话 ID 直接取当前消息的 `unified_msg_origin`。
...
│   ├── access_control.py    # 插件授权管理（群启用控制）与活跃会话记录
...
以下内容是对 v3.6.0 当前源码的静态核对结果，部署和二次开发前应注意：
...
6. 授权管理默认放行全部会话；切换白名单后只有白名单记录可执行插件指令及接收推送，切换黑名单后仅拦截黑名单记录。名单记录会保留在数据库中，切换模式不会删除已有配置。
