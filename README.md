...
| 本地 SQLite | `core/sqlite.py`、`core/bilei_data.py` | 心法别名、避雷记录、事件订阅、使用范围名单和基础数据缓存 |
...
| 指令 | 说明与输出 | 凭据 |
| --- | --- | --- |
| `发言 服务器 角色 [条数] [页数]` | 角色聊天记录，默认 20 条、第 1 页；图片 | Token |
| `统战 [服务器]` | 统战频道统计；文本 | 无 |
| `小药 [心法]` | 小吃小药推荐；图片 | 无 |
...
避雷功能没有内置权限校验。只要能够触发插件指令，当前实现就允许新增、修改和删除记录；公开群聊部署时应在 AstrBot 或消息平台层配置访问控制，也可以先在插件管理页“使用范围”中将模式切换为白名单，只对指定群开放。
...
AstrBot 插件详情页中的“剑网三插件管理”页面提供五个页签：会话绑定、事件订阅、区服别名、心法别名和使用范围。页面可以新增、修改或解除任意会话绑定，查看所有会话的事件总开关及订阅编号，为每个标准区服配置多个别名，并维护心法的 JX3BOX 配装 ID、标准名称和最多 5 个别名。管理页通过 AstrBot Plugin Pages 桥接调用插件 Web API，不直接访问 Dashboard 凭据。

“使用范围”页签用于控制插件在哪些会话可用。默认全部开放，不影响现有部署；切换为白名单后只有名单内的群能触发指令，切换为黑名单后名单内的群被停用。名单条目支持完整会话 ID（AstrBot `unified_msg_origin`）或纯群号；群内触发过插件指令的会话会记录在本地 SQLite，页面会列出最近活跃的群并支持一键加入名单，白名单模式下无需手动抄群号。私聊是否可用由独立开关控制，不随群名单模式变化。被拒会话触发指令时默认静默忽略，也可开启回复提示。使用范围同时约束实时事件推送，被停用的群不再收到任何订阅事件。
...
4. 命中指令后先记录会话来源（管理页“最近活跃的群”数据来源），再按使用范围判断本会话/群是否可用：全部开放直接放行；白名单/黑名单按名单匹配群；私聊按独立开关判定。被拒时按配置静默忽略或回复提示。
5. 允许后停止事件继续传播，由 `_call_with_auto_args()` 通过 `inspect.signature()` 读取参数签名，按注解转换 `int` 和 `float` 后调用处理器。
6. 找不到指令时忽略消息。
...
| 文件 | 生命周期 | 内容 |
| --- | --- | --- |
| AstrBot 插件数据目录下的 `local_data.db` | 运行时创建和维护 | 心法与别名、避雷记录、会话区服绑定、区服别名、事件订阅、使用范围名单及基础数据缓存 |
...
每条消息按 `action` 识别事件，正文优先读取 `detail`，同时兼容 `data`。分发时先按使用范围过滤被停用的会话，再联查会话区服绑定：正文包含 `server` 时仅保留同区服或未绑定区服的接收会话，正文不含 `server` 时不做区服过滤。之后只有会话的 `enabled=1` 且对应 `action_编号=1` 时才发送。会话 ID 直接取当前消息的 `unified_msg_origin`。
